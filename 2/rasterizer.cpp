...
{   
    ...
    // the three points of triangle
    Eigen::Vector2f a = {_v[0].x(), _v[0].y()};
    Eigen::Vector2f b = {_v[1].x(), _v[1].y()};
    Eigen::Vector2f c = {_v[2].x(), _v[2].y()};
    // the 3 edges of triangle
    Eigen::Vector2f ab = b - a;
    Eigen::Vector2f bc = c - b;
    Eigen::Vector2f ca = a - c;马克
    // the point need to be judge
    Eigen::Vector2f p = {x, y};
    // the edges from point to triangle points
    Eigen::Vector2f pa = a - p;
    Eigen::Vector2f pb = b - p;
    Eigen::Vector2f pc = c - p;
    // the x calculated results
    bool d1 = (ab.x()*pa.y() - ab.y()*pa.x() > 0);
    bool d2 = (bc.x()*pb.y() - bc.y()*pb.x() > 0);
    bool d3 = (ca.x()*pc.y() - ca.y()*pc.x() > 0);
    // judge is/isn't the same sign
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    // 取出三角形的三个顶点，将边界点作为bounding box的范围
    for(auto &k:v){
    ...
    for(float x = lmin; x <= rmax; x ++){
        for(float y = bmin; y <= tmax; y ++){
