...
Eigen::Matrix4f get_model_matrix(float rotation_angle)
{
    ...
    //需要转换为弧度制
    rotation_angle = rotation_angle / 360 * 2 * MY_PI;
                                      ...
                                      float zNear, float zFar)
{
    ...
    float b = -n*tan(eye_fov/2);
    float r = aspect_ratio*n*tan(fov/2);
    float l = -aspect_ratio*n*tan(fov/2);
    ...
    scale << 2/(r-l), 0, 0, 0,
             ...
             0, 0, 0, 1;
    // 正交投影矩阵，先平移再缩放
    Eigen::Matrix4f M_ortho = scale * translate;
